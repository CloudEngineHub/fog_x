...
from robodm.ingestion.base import (
    DataIngestionInterface, 
    IngestionConfig, 
    TrajectoryBuilder, 
    BatchProcessor
)
from robodm.ingestion.adapters import (
    PyTorchDatasetAdapter,
    IteratorAdapter, 
    CallableAdapter,
    FileListAdapter
)
from robodm.ingestion.factory import (
...
class MockPyTorchDataset:
    ...
    def __init__(self, size=10):
        self.size = size
        self.data = [{"input": np.random.rand(3, 32, 32), "label": i % 2} for i in range(size)]
...
class TestTrajectoryBuilder:
    ...
    def test_create_trajectory_from_group(self, sample_config, mock_trajectory, temp_dir):
        ...
        result = builder.create_trajectory_from_group(
            ['item1', 'item2'], ingester, output_path
        )
        
        assert result == output_path
        mock_trajectory.add_by_dict.assert_has_calls([
            call({"data": "transformed_item1", "value": mock_trajectory.add_by_dict.call_args_list[0][0][0]["value"]}, timestamp=0, time_unit="ms"),
            call({"data": "transformed_item2", "value": mock_trajectory.add_by_dict.call_args_list[1][0][0]["value"]}, timestamp=100, time_unit="ms")
        ])
        mock_trajectory.close.assert_called_once()
    
    def test_create_trajectory_with_transform_error(self, sample_config, mock_trajectory, temp_dir):
    ...
    def test_create_trajectory_with_max_items(self, sample_config, mock_trajectory, temp_dir):
...
class TestBatchProcessor:
    ...
    def test_process_trajectory_groups(self, sample_config, mock_trajectory, temp_dir):
        ...
        with patch.object(processor.builder, 'create_trajectory_from_group') as mock_create:
    ...
    def test_process_trajectory_groups_with_errors(self, sample_config, temp_dir):
...
class TestPyTorchDatasetAdapter:
    ...
    def test_init_invalid_dataset(self):
        ...
        with pytest.raises(ValueError, match="must implement __len__ and __getitem__"):
    ...
    def test_get_trajectory_filename_custom(self):
        ...
        adapter = PyTorchDatasetAdapter(dataset, trajectory_name_fn=custom_name_fn)
...
class TestFactoryFunctions:
    ...
    def test_create_vla_dataset_from_source(self, mock_mkdtemp, mock_parallel_ingester):
        ...
        result = create_vla_dataset_from_source(
            dataset,
    ...
    def test_create_vla_dataset_from_pytorch_dataset(self):
        ...
        with patch('robodm.ingestion.factory.create_vla_dataset_from_source') as mock_create:
            create_vla_dataset_from_pytorch_dataset(
                dataset,
...
class TestParallelDataIngester:
    ...
    def test_init_ray_initialization(self, mock_ray_init, mock_is_initialized, sample_config):
    ...
    def test_init_creates_output_directory(self, mock_makedirs, sample_config):
        ...
        mock_makedirs.assert_called_once_with(sample_config.output_directory, exist_ok=True)
    ...
    def test_ingest_data_empty_items(self, sample_config):
        ...
        with patch('robodm.ingestion.parallel.logger') as mock_logger:
            result = parallel_ingester.ingest_data(ingester, return_vla_dataset=False)
...
class TestEdgeCases:
    ...
    def test_trajectory_builder_validation_failure(self, sample_config, mock_trajectory, temp_dir):
        ...
        builder = TrajectoryBuilder(sample_config)
        ingester = ValidatingIngester(['good_item', 'bad_item', 'another_good'])
