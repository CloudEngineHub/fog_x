...
import numpy as np
import robodm
from robodm.loader.vla import RayVLALoader, LoadingMode, SliceConfig
...
def create_test_trajectories(temp_dir: Path, num_trajectories: int = 3):
    ...
    for i in range(num_trajectories):
        ...
        # Create sample data
        observations_image = np.random.randint(0, 255, (traj_length, 640, 480, 3), dtype=np.uint8)
...
def test_metadata_loading():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert metadata_manager.exists(), "Metadata file should have been created"
        ...
        if slices:
            ...
            if 'actions' in first_slice:
                logger.info(f"First slice action shape: {first_slice['actions'].shape}")
        
        logger.info("\n=== Testing metadata filtering ===")
        # Test filtering by length
        long_trajectories = metadata_manager.filter_by_length(min_length=150)
        logger.info(f"Found {len(long_trajectories)} trajectories with length >= 150")
        
        for meta in long_trajectories:
            logger.info(f"  - {Path(meta.file_path).name}: length={meta.trajectory_length}")
