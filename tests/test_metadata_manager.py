...
def sample_trajectory_metadata():
    ...
    return [
        TrajectoryMetadata(
            ...
            feature_shapes={"action": [7], "observation/images/cam_high": [128, 128, 3]},
            feature_dtypes={"action": "float32", "observation/images/cam_high": "uint8"},
            file_size=1024000,
        ...
        TrajectoryMetadata(
            ...
            feature_shapes={"action": [7], "observation/state/joint_pos": [7]},
            feature_dtypes={"action": "float32", "observation/state/joint_pos": "float32"},
            file_size=2048000,
...
class TestTrajectoryMetadata:
    ...
    def test_roundtrip_conversion(self):
        ...
        original = TrajectoryMetadata(
            ...
            feature_shapes={"action": [7], "observation": [128, 128, 3]},
            feature_dtypes={"action": "float32", "observation": "uint8"},
            file_size=1024,
            last_modified=datetime(2023, 1, 1, 12, 0, 0)
        )
...
class TestMetadataManager:
    ...
    def test_save_metadata(self, temp_dataset_dir, sample_trajectory_metadata):
        ...
        assert list(df.columns) == [
            'file_path', 'trajectory_length', 'feature_keys', 'feature_shapes',
            'feature_dtypes', 'file_size', 'last_modified', 'checksum'
        ]
    ...
    def test_save_metadata_exception_handling(self, temp_dataset_dir, sample_trajectory_metadata):
        """Test exception handling during save."""
        manager = MetadataManager(temp_dataset_dir)
        
        with patch('pandas.DataFrame.to_parquet', side_effect=Exception("Save failed")):
            with patch('robodm.metadata_manager.logger') as mock_logger:
    ...
    def test_load_metadata_caching(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_load_metadata_force_reload(self, temp_dataset_dir, sample_trajectory_metadata):
        ...
        with patch('pandas.read_parquet', return_value=pd.DataFrame()) as mock_read:
    ...
    def test_get_trajectory_metadata(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_get_trajectory_metadata_not_found(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_get_trajectory_metadata_path_normalization(self, temp_dataset_dir, sample_trajectory_metadata):
        """Test path normalization in get_trajectory_metadata."""
        manager = MetadataManager(temp_dataset_dir)
        manager.save_metadata(sample_trajectory_metadata)
        
        with patch('pathlib.Path.resolve', return_value=Path("/path/to/traj1.vla")):
            metadata = manager.get_trajectory_metadata("../path/to/traj1.vla")
            assert metadata is not None
    
    def test_update_metadata_no_existing(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_update_metadata_existing_file(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_update_metadata_add_new_trajectories(self, temp_dataset_dir, sample_trajectory_metadata):
        """Test adding new trajectories to existing metadata."""
        manager = MetadataManager(temp_dataset_dir)
        manager.save_metadata(sample_trajectory_metadata[:1])  # Save only first trajectory
    ...
    def test_remove_metadata(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_remove_metadata_no_file(self, temp_dataset_dir):
        ...
        with patch('robodm.metadata_manager.logger') as mock_logger:
            manager.remove_metadata(["/path/to/traj1.vla"])
            mock_logger.warning.assert_called_once_with("No metadata file to remove from")
    
    def test_remove_metadata_path_normalization(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_get_all_metadata(self, temp_dataset_dir, sample_trajectory_metadata):
        ...
        assert all(isinstance(meta, TrajectoryMetadata) for meta in all_metadata)
        assert all_metadata[0].file_path == "/path/to/traj1.vla"
        assert all_metadata[1].file_path == "/path/to/traj2.vla"
    
    def test_filter_by_length(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_get_statistics(self, temp_dataset_dir, sample_trajectory_metadata):
        ...
        assert stats['total_trajectories'] == expected_stats['total_trajectories']
        ...
        assert set(stats['unique_feature_keys']) == expected_stats['unique_feature_keys']
...
class TestEdgeCases:
    ...
    def test_concurrent_access_simulation(self, temp_dataset_dir, sample_trajectory_metadata):
    ...
    def test_large_feature_shapes(self, temp_dataset_dir):
        ...
        metadata = TrajectoryMetadata(
            ...
            feature_dtypes={k: "float32" for k in complex_shapes.keys()},
        ...
        retrieved = manager.get_trajectory_metadata("/path/to/complex_traj.vla")
        assert retrieved is not None
