...
class TestRLDSLoader:
    ...
    def test_init_without_tensorflow(self):
        ...
        with patch.dict('sys.modules', {'tensorflow': None}):
            with pytest.raises(ImportError, match="Please install tensorflow and tensorflow_datasets"):
    ...
    def test_init_basic(self, mock_tf, mock_tfds, mock_tfds_builder):
        ...
        loader = RLDSLoader("/path/to/dataset", split="train", batch_size=4, shuffling=False)
        ...
        mock_tfds.builder_from_directory.assert_called_once_with("/path/to/dataset")
    ...
    def test_init_with_shuffling(self, mock_tf, mock_tfds, mock_tfds_builder):
        ...
        loader = RLDSLoader("/path/to/dataset", shuffling=True, shuffle_buffer=20)
        ...
        mock_tfds_builder.as_dataset.return_value.shuffle.assert_called_once_with(20)
    ...
    @patch('robodm.loader.rlds.tfds')
    @patch('robodm.loader.rlds.tf')
    def test_get_batch(self, mock_tf, mock_tfds, mock_tfds_builder, sample_trajectory_data):
        ...
        with patch.object(loader, '_convert_traj_to_numpy', side_effect=lambda x: f"converted_{id(x)}") as mock_convert:
    ...
    @patch('robodm.loader.rlds.tfds')
    @patch('robodm.loader.rlds.tf')
    def test_get_batch_stop_iteration(self, mock_tf, mock_tfds, mock_tfds_builder):
        ...
        with patch.object(loader, '_convert_traj_to_numpy', return_value="converted"):
            batch = loader.get_batch()
    ...
    @patch('robodm.loader.rlds.tfds')
    @patch('robodm.loader.rlds.tf')
    def test_next(self, mock_tf, mock_tfds, mock_tfds_builder, sample_trajectory_data):
        ...
        with patch.object(loader, '_convert_traj_to_numpy', return_value="converted_traj") as mock_convert:
            result = next(loader)
    ...
    @patch('robodm.loader.rlds.tfds')
    @patch('robodm.loader.rlds.tf')
    def test_getitem(self, mock_tf, mock_tfds, mock_tfds_builder, sample_trajectory_data):
        ...
        mock_skip_take = Mock()
        mock_skip_take.__iter__ = Mock(return_value=iter([sample_trajectory_data]))
        mock_dataset.skip.return_value.take.return_value = mock_skip_take
        
        loader = RLDSLoader("/path/to/dataset")
        
        with patch.object(loader, '_convert_traj_to_numpy', return_value="converted_item") as mock_convert:
            result = loader[5]
...
class TestRLDSLoaderEdgeCases:
    ...
    def test_large_shuffle_buffer(self, mock_tf, mock_tfds, mock_tfds_builder):
        ...
        loader = RLDSLoader("/path/to/dataset", shuffle_buffer=10000, shuffling=True)
        
        # Verify shuffle was called with large buffer
        mock_tfds_builder.as_dataset.return_value.shuffle.assert_called_once_with(10000)
    
    @patch('robodm.loader.rlds.tfds')
    @patch('robodm.loader.rlds.tf')
    def test_index_tracking_with_shuffling(self, mock_tf, mock_tfds, mock_tfds_builder):
...
class TestRLDSLoaderIntegration:
    ...
    def test_full_iteration_cycle(self, mock_tf, mock_tfds, mock_tfds_builder):
        ...
        mock_tfds_builder.as_dataset.return_value.__len__ = Mock(return_value=3)
        loader = RLDSLoader("/path/to/dataset", shuffling=False)
        loader.length = 3
        ...
        mock_iterator = Mock()
        mock_iterator.__next__ = Mock(side_effect=[sample_data, sample_data, sample_data, StopIteration])
        loader.iterator = mock_iterator
    ...
    @patch('robodm.loader.rlds.tfds')
    @patch('robodm.loader.rlds.tf')
    def test_batch_and_single_item_consistency(self, mock_tf, mock_tfds, mock_tfds_builder, sample_trajectory_data):
