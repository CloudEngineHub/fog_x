...
def mock_ray_vla_loader():
    ...
    with patch('robodm.dataset.RayVLALoader') as mock_loader_class:
        ...
        mock_loader.schema.return_value = {
            'observation/images/cam_high': {'shape': (10, 128, 128, 3), 'dtype': 'float32'},
            'action': {'shape': (10, 7), 'dtype': 'float32'}
        }
...
class TestVLADataset:
    ...
    def test_create_trajectory_dataset(self, mock_ray_vla_loader, sample_vla_files):
    ...
    def test_map(self, mock_ray_vla_loader, sample_vla_files):
        ...
        dataset.loader.dataset.map.assert_called_once_with(map_fn, batch_format="numpy")
    ...
    def test_get_stats_trajectory_mode(self, mock_ray_vla_loader, sample_vla_files):
        """Test get_stats for trajectory mode."""
        dataset = VLADataset(path=sample_vla_files[0], mode=LoadingMode.TRAJECTORY)
        stats = dataset.get_stats()
        
        expected_keys = ['mode', 'return_type', 'total_items', 'sample_keys', 'trajectory_length']
    ...
    def test_get_stats_slice_mode(self, mock_ray_vla_loader, sample_vla_files):
        ...
        expected_keys = ['mode', 'return_type', 'total_items', 'sample_keys', 'slice_length']
        assert all(key in stats for key in expected_keys)
        assert stats['mode'] == 'slice'
        assert stats['slice_length'] == 10
    
    def test_get_stats_empty_dataset(self, mock_ray_vla_loader, sample_vla_files):
    ...
    def test_getitem_not_supported(self, mock_ray_vla_loader, sample_vla_files):
...
class TestUtilityFunctions:
    ...
    @pytest.mark.skipif(not RAY_AVAILABLE, reason="Ray not available")
    def test_load_trajectory_dataset(self, mock_ray_vla_loader, sample_vla_files):
    ...
    def test_split_dataset_invalid_fractions(self, mock_ray_vla_loader, sample_vla_files):
...
class TestEdgeCases:
    ...
    @pytest.mark.skipif(not RAY_AVAILABLE, reason="Ray not available")
    def test_string_mode_conversion(self, mock_ray_vla_loader, sample_vla_files):
    ...
    @pytest.mark.skipif(not RAY_AVAILABLE, reason="Ray not available")
    def test_multiple_operations_chaining(self, mock_ray_vla_loader, sample_vla_files):
