...
class TestCollectTrajectories:
    ...
    def test_collect_trajectories_large_dataset(self):
        ...
        trajectories = executor._collect_trajectories(mock_dataset, max_trajectories=100)
    ...
    def test_collect_trajectories_fallback(self):
        ...
        mock_dataset.take.assert_called_once_with(100)  # Default max_trajectories is 100
    ...
    def test_collect_trajectories_complete_failure(self):
        ...
        with pytest.raises(RuntimeError, match="Failed to collect trajectories"):
...
class TestApplyFilter:
    ...
    def test_apply_filter_success(self, mock_ray_dataset):
        ...
        filtered_dataset = executor.apply_filter(mock_ray_dataset, simple_filter)
...
class TestApplyAggregation:
    ...
    def test_apply_aggregation_success(self):
        ...
        trajectories = [
            ...
            {"metadata": {"scene": "office"}},
            {"metadata": {"scene": "kitchen"}},
        ]
        
        with patch.object(executor, '_collect_trajectories', return_value=trajectories):
            mock_dataset = Mock()
            
            def count_by_scene(trajs: List[Dict[str, Any]]) -> Dict[str, int]:
                from collections import Counter
                scenes = [t.get("metadata", {}).get("scene", "unknown") for t in trajs]
    ...
    def test_apply_aggregation_failure(self):
        ...
        with patch.object(executor, '_collect_trajectories', side_effect=Exception("Collection failed")):
            mock_dataset = Mock()
            
            def simple_agg(trajs: List[Dict[str, Any]]) -> int:
                return len(trajs)
            
            with pytest.raises(RuntimeError, match="Failed to apply aggregation"):
...
class TestGetExecutionStats:
    ...
    def test_get_execution_stats_ray_initialized(self, mock_cluster_resources, mock_ray_init):
