...
from robodm.utils.flatten import (
    data_to_tf_schema,
    _flatten,
    _flatten_dict,
...
class TestDataToTfSchema:
    ...
    def test_simple_data(self):
        ...
        mock_ft_instance.to_tf_feature_type.assert_called_with(first_dim_none=True)
...
class TestRecursivelyReadHdf5Group:
    ...
    def test_read_group(self):
        ...
        try:
            ...
            assert np.array_equal(result['subgroup']['dataset3'], np.array([7, 8, 9]))
    ...
    def test_read_complex_structure(self):
        ...
        try:
            ...
            with h5py.File(tmp_path, 'w') as f:
                ...
                images_group.create_dataset('cam_high', data=np.random.rand(128, 128, 3))
                images_group.create_dataset('cam_low', data=np.random.rand(64, 64, 3))
                
                # State subgroup
                state_group = obs_group.create_group('state')
                state_group.create_dataset('joint_pos', data=np.array([0.1, 0.2, 0.3]))
                state_group.create_dataset('joint_vel', data=np.array([1.0, 2.0, 3.0]))
                
                # Action group
                action_group = f.create_group('action')
                action_group.create_dataset('joint_action', data=np.array([0.5, -0.5]))
            ...
            assert np.array_equal(state['joint_pos'], np.array([0.1, 0.2, 0.3]))
            assert np.array_equal(state['joint_vel'], np.array([1.0, 2.0, 3.0]))
            
            # Verify action
            action = result['action']
            assert isinstance(action, dict)
            assert 'joint_action' in action
            assert np.array_equal(action['joint_action'], np.array([0.5, -0.5]))
