...
from robodm.agent import Agent, Planner, Executor
...
def sample_trajectory():
    ...
    return {
        "observation/image": np.random.randint(0, 255, (10, 64, 64, 3), dtype=np.uint8),
        "observation/state": np.random.randn(10, 7),
        "action": np.random.randn(10, 3),
        "metadata": {"episode_id": 1, "scene": "kitchen"}
    }
...
def sample_trajectories(sample_trajectory):
    ...
    for i in range(5):
        ...
        traj["metadata"] = {"episode_id": i, "scene": "kitchen" if i < 3 else "office"}
...
class TestAgent:
    ...
    def test_agent_init(self, mock_executor_class, mock_planner_class, mock_ray_dataset):
        ...
        mock_planner_class.assert_called_once_with(llm_model="test-model", tools_manager=agent.tools_manager)
        mock_executor_class.assert_called_once_with(tools_manager=agent.tools_manager)
    
    @patch('robodm.agent.agent.Planner')
    @patch('robodm.agent.agent.Executor')
    def test_agent_filter(self, mock_executor_class, mock_planner_class, mock_ray_dataset):
        ...
        mock_planner.generate_filter_function.assert_called_once_with("trajectories with occlusion", dataset=mock_ray_dataset)
        mock_executor.apply_filter.assert_called_once_with(mock_ray_dataset, mock_filter_func)
    
    @patch('robodm.agent.agent.Planner')
    @patch('robodm.agent.agent.Executor')
    def test_agent_map(self, mock_executor_class, mock_planner_class, mock_ray_dataset):
        ...
        mock_planner.generate_map_function.assert_called_once_with("add frame differences", dataset=mock_ray_dataset)
        mock_executor.apply_map.assert_called_once_with(mock_ray_dataset, mock_map_func)
    
    def test_agent_count(self, mock_ray_dataset):
        """Test Agent count functionality."""        
        with patch('robodm.agent.agent.Planner'), patch('robodm.agent.agent.Executor'):
            agent = Agent(mock_ray_dataset)
            count = agent.count()
    ...
    def test_agent_inspect_schema(self, mock_ray_dataset):
        ...
        with patch('robodm.agent.agent.Planner') as mock_planner_class:
            ...
            mock_schema_info = {
                ...
                "shapes": {"observation/image": [10, 64, 64, 3]},
                "dtypes": {"observation/image": "uint8"},
                "has_images": True,
    ...
    def test_agent_with_tools_config(self, mock_ray_dataset):
        ...
        tools_config = {
            "tools": {
                "robo2vlm": {"temperature": 0.05, "max_tokens": 512}
            },
    ...
    def test_agent_describe_dataset(self, mock_ray_dataset):
        ...
        with patch('robodm.agent.agent.Planner') as mock_planner_class:
            ...
            mock_schema_info = {
                ...
                "shapes": {"observation/image": [10, 64, 64, 3]},
                "dtypes": {"observation/image": "uint8"},
                "sample_values": {"metadata": {"scene": "kitchen"}},
                "has_images": True,
                "image_keys": ["observation/image"],
...
class TestIntegration:
    ...
    def test_error_propagation(self, mock_ray_dataset):
        ...
        with patch('robodm.agent.agent.Planner') as mock_planner_class:
            mock_planner = Mock()
            mock_planner.generate_filter_function.side_effect = RuntimeError("LLM failed")
            mock_planner_class.return_value = mock_planner
