...
class ToolRegistry:
    ...
    def configure_tool(self, tool_name: str, **config):
        ...
        keys_to_remove = [key for key in self._tool_instances.keys() if key.startswith(f"{tool_name}_")]
        for key in keys_to_remove:
            del self._tool_instances[key]
    
    def get_tools_namespace(self, tool_names: Optional[List[str]] = None, **tool_configs) -> Dict[str, BaseTool]:
        """
    ...
    def get_tools_documentation(self, tool_names: Optional[List[str]] = None) -> str:
        """
