...
def create_vision_config(model: str = "qwen2.5-7b", temperature: float = 0.05, max_tokens: int = 512) -> Dict[str, Any]:
    """
...
def create_custom_config(
    ...
    if disabled_tools:
        ...
        config["disabled_tools"] = [tool for tool in all_tools if tool not in enabled_tools]
    ...
    if not isinstance(tools_config, dict):
        ...
        for tool_name, tool_config in tools_config.items():
            if not isinstance(tool_config, dict):
                issues.append(f"Configuration for tool '{tool_name}' must be a dictionary")
                continue
            ...
            if tool_name == "robo2vlm":
                ...
                if not isinstance(temp, (int, float)) or temp < 0 or temp > 2.0:
                    issues.append(f"robo2vlm temperature must be between 0 and 2.0, got {temp}")
                
                max_tokens = tool_config.get("max_tokens", 256)
                if not isinstance(max_tokens, int) or max_tokens <= 0:
                    issues.append(f"robo2vlm max_tokens must be positive integer, got {max_tokens}")
            
            elif tool_name == "analyze_image":
                blur_thresh = tool_config.get("blur_threshold", 100.0)
                if not isinstance(blur_thresh, (int, float)) or blur_thresh <= 0:
                    issues.append(f"analyze_image blur_threshold must be positive, got {blur_thresh}")
                
                bright_thresh = tool_config.get("brightness_threshold", 0.3)
                if not isinstance(bright_thresh, (int, float)) or not 0 <= bright_thresh <= 1:
                    issues.append(f"analyze_image brightness_threshold must be between 0 and 1, got {bright_thresh}")
            
            elif tool_name == "analyze_trajectory":
                anom_thresh = tool_config.get("anomaly_threshold", 3.0)
                if not isinstance(anom_thresh, (int, float)) or anom_thresh <= 0:
                    issues.append(f"analyze_trajectory anomaly_threshold must be positive, got {anom_thresh}")
                
                min_len = tool_config.get("min_length", 10)
                if not isinstance(min_len, int) or min_len <= 0:
                    issues.append(f"analyze_trajectory min_length must be positive integer, got {min_len}")
                
                smooth_win = tool_config.get("smoothing_window", 5)
                if not isinstance(smooth_win, int) or smooth_win <= 0:
                    issues.append(f"analyze_trajectory smoothing_window must be positive integer, got {smooth_win}")
    ...
    if not isinstance(disabled_tools, list):
        ...
        for tool in disabled_tools:
            if not isinstance(tool, str):
                issues.append(f"Disabled tool name must be string, got {type(tool)}")
