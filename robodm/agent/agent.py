...
class Agent:
    ...
    def __init__(self, dataset: Dataset, llm_model: str = "qwen2.5-7b", tools_config: Optional[Dict[str, Any]] = None):
        """
        ...
        self.planner = Planner(llm_model=llm_model, tools_manager=self.tools_manager)
    ...
    def filter(self, prompt: str) -> Dataset:
        ...
        filter_func = self.planner.generate_filter_function(prompt, dataset=self.dataset)
    ...
    def map(self, prompt: str) -> Dataset:
        ...
        map_func = self.planner.generate_map_function(prompt, dataset=self.dataset)
    ...
    def aggregate(self, prompt: str) -> Any:
        ...
        agg_func = self.planner.generate_aggregation_function(prompt, dataset=self.dataset)
    ...
    def analyze(self, prompt: str) -> str:
        ...
        analysis_func = self.planner.generate_analysis_function(prompt, dataset=self.dataset)
