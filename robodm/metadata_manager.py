...
class MetadataManager:
    ...
    def __init__(
        ...
        metadata_filename: str = "trajectory_metadata.parquet",
    ):
        """
    ...
    def load_metadata(self, force_reload: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Loaded metadata for {len(self._metadata_cache)} trajectories"
            )
            return self._metadata_cache
    ...
    def save_metadata(self, metadata_list: List[TrajectoryMetadata]) -> None:
        ...
        try:
            ...
            logger.info(
                f"Saved metadata for {len(df)} trajectories to {self.metadata_path}"
            )
        except Exception as e:
    ...
    def get_all_metadata(self) -> List[TrajectoryMetadata]:
        ...
        df = self.load_metadata()
        return [
            TrajectoryMetadata.from_dict(row.to_dict())
