...
import robodm
from robodm.metadata_manager import TrajectoryMetadata, MetadataManager
...
def extract_trajectory_metadata(file_path: str, compute_checksum: bool = False) -> TrajectoryMetadata:
    """
    ...
    try:
        ...
        for key, value in data.items():
            if hasattr(value, 'shape'):
                # For numpy arrays
                feature_shapes[key] = list(value.shape[1:])  # Exclude time dimension
...
def build_dataset_metadata(
    ...
    for i, file_path in enumerate(trajectory_files):
        try:
            logger.debug(f"Processing {i+1}/{len(trajectory_files)}: {file_path}")
            metadata = extract_trajectory_metadata(str(file_path), compute_checksums)
...
def update_dataset_metadata(
    ...
    existing_metadata = {meta.file_path: meta for meta in manager.get_all_metadata()}
    ...
    for file_path in trajectory_files:
        ...
        if (file_path_str not in existing_metadata or 
            existing_metadata[file_path_str].last_modified < last_modified):
            try:
