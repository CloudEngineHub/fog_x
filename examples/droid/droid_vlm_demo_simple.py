...
class DROIDSuccessDetector:
    ...
    def analyze_trajectory_frames(self,
        ...
        print(
            f"  Analyzing {len(frames)} frames from {primary_camera} (sampling every {sample_rate} frames)"
        )

        # Sample frames for analysis
        frame_indices = list(range(
            0, len(frames), sample_rate))[:5]  # Limit to 5 frames for demo
    ...
    def _assess_trajectory_success(self, frame_analyses: List[Dict]) -> Dict:
        ...
        for analysis in frame_analyses:
            ...
            if ("yes" in responses.get(
                    "Is the robot gripper holding any object?", "").lower()):
    ...
    def compare_trajectories(self, success_paths: List[str],
        ...
        for path in success_paths:
            if os.path.exists(path):
                ...
                print(
                    f"  VLM Prediction: {'SUCCESS' if assessment['is_successful'] else 'FAILURE'}"
                )
                print(
                    f"  Success indicators: {assessment['success_indicators']}"
                )
                print(
        ...
        if total_success > 0:
            ...
            print(
                f"Success detection accuracy: {success_accuracy:.0%} ({correct_success}/{total_success})"
            )

        if total_failure > 0:
            failure_accuracy = correct_failure / total_failure
            print(
                f"Failure detection accuracy: {failure_accuracy:.0%} ({correct_failure}/{total_failure})"
            )
...
def main():
    ...
    print(
        f"Found {len(success_vla_paths)} successful and {len(failure_vla_paths)} failed trajectories"
    )
