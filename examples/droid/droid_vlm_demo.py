...
class DROIDSuccessDetector:
    ...
    def analyze_trajectory_frames(self, trajectory_path: str, sample_rate: int = 10) -> Dict:
        """
        ...
        for idx in frame_indices:
            ...
            for prompt in prompts:
                try:
                    ...
                    print(f"Error analyzing frame {idx} with prompt '{prompt}': {e}")
    ...
    def _assess_trajectory_success(self, frame_analyses: List[Dict]) -> Dict:
        ...
        for analysis in frame_analyses:
            ...
            if "yes" in responses.get("Is the robot gripper holding any object? Answer yes or no.", "").lower():
                success_indicators += 1
            
            # Check for failure signs
            failure_response = responses.get("Are there any signs of failure (dropped objects, collision, stuck position)?", "")
            if any(word in failure_response.lower() for word in ["yes", "dropped", "collision", "stuck"]):
        ...
        success_rate = success_indicators / (total_frames * 2) if total_frames > 0 else 0  # *2 for two success questions
    ...
    def compare_trajectories(self, success_paths: List[str], failure_paths: List[str]):
        ...
        if total_success > 0:
            ...
            print(f"Success detection accuracy: {success_accuracy:.2%} ({correct_success}/{total_success})")
        
        if total_failure > 0:
            failure_accuracy = correct_failure / total_failure
            print(f"Failure detection accuracy: {failure_accuracy:.2%} ({correct_failure}/{total_failure})")
...
def main():
    ...
    print("\n" + "="*60)
    print("Demo complete! The robo2vlm tool successfully analyzed DROID trajectories.")
    print("\nKey insights:")
    print("- VLM can detect task completion indicators in robotic trajectories")
