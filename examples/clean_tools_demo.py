...
def demo_clean_architecture():
    ...
    if isinstance(result, dict) and 'blur' in result:
        print(f"Direct blur analysis: {result['blur'].get('is_blurry', 'N/A')}")
    else:
    ...
    if isinstance(stats, dict) and 'length' in stats:
        print(f"Direct trajectory stats: length={stats['length']}, mean={np.array(stats['mean'])[:2]}")
    else:
    ...
    if "analyze_image" in manager.list_tools():
        ...
        if isinstance(managed_result, dict) and 'blur' in managed_result:
            print(f"Managed blur analysis: {managed_result['blur'].get('is_blurry', 'N/A')}")
        else:
...
def demo_configuration_system():
    ...
    configs = {
        "Vision-focused": create_vision_config(),
        "Analysis-focused": create_analysis_config(),
        "Minimal": create_minimal_config(),
        "Custom": create_custom_config(
            enabled_tools=["analyze_image", "analyze_trajectory"],
            tool_parameters={
                "analyze_image": {"blur_threshold": 60.0},
                "analyze_trajectory": {"anomaly_threshold": 2.0}
            }
        )
    }
    ...
    for name, config in configs.items():
        ...
        try:
            ...
            if "analyze_image" in manager.list_tools():
                ...
                if isinstance(result, dict) and 'blur' in result:
                    print(f"Blur threshold: {result['blur'].get('threshold', 'N/A')}")
                else:
...
def demo_custom_tool_registration():
    ...
    class SmoothnessCalculatorTool(BaseTool):
        ...
        def get_metadata(cls) -> ToolMetadata:
            return ToolMetadata(
                ...
                description="Calculate trajectory smoothness using local variance",
                examples=[
    ...
    class MotionClassifierTool(BaseTool):
        ...
        def __init__(self, velocity_threshold: float = 1.0, acceleration_threshold: float = 2.0, **kwargs):
            super().__init__(velocity_threshold=velocity_threshold, 
                           acceleration_threshold=acceleration_threshold, **kwargs)
        ...
        def get_metadata(cls) -> ToolMetadata:
            return ToolMetadata(
                ...
                parameters={"velocity_threshold": 1.0, "acceleration_threshold": 2.0}
            )
    ...
    if "calculate_smoothness" in manager.list_tools():
        ...
        test_smooth = np.sin(np.linspace(0, 10, 50))[:, None] * np.array([1, 0.5, 0.2])
...
def demo_dynamic_configuration():
    ...
    if hasattr(manager, 'enable_tool'):
        ...
        print("Dynamic tool enabling not available - using config-based approach")
        # Create new manager with different config
        config = create_custom_config(enabled_tools=["robo2vlm", "analyze_image"])
        manager = ToolsManager(config=config)
    ...
    if "analyze_image" in manager.list_tools():
        ...
        result = analyze_img(test_image, "blur")
        if isinstance(result, dict) and 'blur' in result:
            print(f"Current blur threshold: {result['blur'].get('threshold', 'N/A')}")
